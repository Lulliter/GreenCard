...
# PLOT INPUTS
year_dat <- tibble(year = c(28, 30),
						 label = c( "Trump", "HireAmer"))
qualitative <- c('#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99',
...
gg_static_region <- gg_gc_static_region %>%
	...
	geom_label(data = year_dat, aes(x = year, y = c(10000, 7000), label = label),
				  angle = 0, color = "black") +
	...
	scale_x_continuous(breaks = 1:54, labels = c("2014-10", "2014-11", "2014-12",
																"2015-1", "2015-2", "2015-3", "2015-4", "2015-5", "2015-6",
																"2015-7", "2015-8", "2015-9", "2015-10", "2015-11","2015-12",
	...
	labs(x = "", y = "Number of Employment-Based Green Cards\n",
...
gg_static_region <- gg_gc_static_region %>%
	...
	geom_label(data = year_dat, aes(x = year, y = c(18000, 15000), label = label),
				  angle = 0, color = "black") +
...
gg_denied_region <- gg_denied_region_data %>%
	ggplot(aes(year_month_n, n  )) +
	geom_vline(data = year_dat, aes(xintercept = year), alpha = 0.15) +
	geom_label(data = year_dat, aes(x = year, y = c(500, 400), label = label),
				  angle = 0, color = "black") +
	geom_line(aes(linetype = region, color = region)) +
	# geom_area() +
	theme_minimal() +
