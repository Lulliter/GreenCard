...
p_load(tidyverse, readxl, writexl, here,lubridate, janitor,
...
countries <- c( "VENEZUELA", "SYRIA", "NORTH KOREA", "YEMEN", "LIBYA", "IRAN", "SOMALIA", # ban
 ...
 # dplyr::filter(FW_INFO_BIRTH_COUNTRY %in% countries) %>%
  dplyr::rename(COUNTRY_OF_BIRTH = FW_INFO_BIRTH_COUNTRY) %>%
	dplyr::mutate(REGION_BIRTH = case_when(
		COUNTRY_OF_BIRTH %in% c( "BURMA (MYANMAR)", "INDONESIA",
		...
		COUNTRY_OF_BIRTH %in% c("BHUTAN", "SRI LANKA", "MALDIVES", "NEPAL", "PAKISTAN",
										"AFGHANISTAN", "BANGLADESH", "INDIA") ~ "SouthAsia",

		COUNTRY_OF_BIRTH %in% c("MOLDOVA","GEORGIA","PORTUGAL", "ROMANIA","ARMENIA", "BELARUS", "KYRGYZSTAN",
										...
										"DENMARK", "KAZAKHSTAN", "NORWAY") ~ "Europe & CentrAsia",

		COUNTRY_OF_BIRTH %in% c("MONTSERRAT", "NETHERLANDS ANTILLES", "SINT MAARTEN","ST VINCENT", "SURINAME", "NICARAGUA", "PANAMA",
										...
										"TRINIDAD AND TOBAGO") ~ "LatAm & Caribb",

		COUNTRY_OF_BIRTH %in% c("EGYPT", "JORDAN", "KUWAIT", "PALESTINIAN TERRITORIES", "QATAR", "BAHRAIN",
										"IRAQ", "MOROCCO","OMAN", "UNITED ARAB EMIRATES", "TUNISIA", "ALGERIA", "ISRAEL",
										"SYRIA", "YEMEN", "LIBYA", "IRAN", "SAUDI ARABIA",
										"LEBANON", "PALESTINE") ~ "MiddleEast & NorthAfr",

		COUNTRY_OF_BIRTH %in% c( "BENIN", "IVORY COAST","MAURITIUS",
										 ...
										 "MALI", "MOZAMBIQUE", "SOUTH AFRICA", "ZIMBABWE") ~ "Sub-Sahar Afr",
...
recap<- FY_15_19 %>% group_by(COUNTRY_OF_BIRTH) %>% count()
...
dat <- FY_15_19 %>%
	dplyr::mutate(YEAR = lubridate::year (DECISION_DATE),
	...
	dplyr::mutate(SPECIAL_COUNTRY = case_when(
		...
		.$COUNTRY_OF_BIRTH %in% c("EGYPT", "JORDAN", "KUWAIT", "PALESTINIAN TERRITORIES", "QATAR", "BAHRAIN",
										  "IRAQ", "MOROCCO","OMAN", "UNITED ARAB EMIRATES", "TUNISIA", "ALGERIA", "ISRAEL",
										  "SAUDI ARABIA","LEBANON", "PALESTINE") ~ "MiddleEast & NorthAfr",
...
dat$EDUC_LEVEL <- factor(dat$EDUC_LEVEL, levels = c("None", "HighSchool_Other",
